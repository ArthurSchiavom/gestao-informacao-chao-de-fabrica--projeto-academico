...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">Chão de Fábrica<xsl:for-each select="child::*/*">
        ...
        <xsl:if test="count(*) > 0"><xsl:text>&#xa;&#x9;</xsl:text><xsl:value-of select="concat(upper-case(substring(local-name(),1,1)),
            ...
            <xsl:for-each select="child::*"><xsl:text>&#xa;&#x9;&#x9;</xsl:text><!-- Verifica se tem atributo e se tiver escreve no titulo do elemento -->
                ...
                </xsl:choose><xsl:apply-templates select="*" mode="Elemento"><xsl:with-param name="indent" select="'&#x9;&#x9;&#x9;'" /></xsl:apply-templates><!-- Chama o template que trata dos value objects de cada elemento de dominio-->
            </xsl:for-each>
        </xsl:if>
    </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template match="*" mode="Elemento">
        ...
        <xsl:choose>
            <xsl:when test="count(*)>0"> - [<xsl:apply-templates select="*" mode="Elemento"><xsl:with-param name="indent" select="concat($indent,'&#x9;')" /></xsl:apply-templates><xsl:text>&#xa;</xsl:text><xsl:value-of select="$indent"/>]</xsl:when>
